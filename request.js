...
function userRequestHandler(req, userRes) {
  ...
  fetchReqData(req).then(routeReq).catch(function(err) {
    console.log(err);
  });

  //get request body
  function fetchReqData(req) {
  ...
  function dealWithLocalResponse(callback) {
    userRule.dealLocalResponse(req, reqData, function(
      statusCode,
      resHeader,
      resBody
    ) {
          ...
          [
            ...
            function(callback) {
              if (userRule.replaceServerResData) {
                logUtil.printLog(
                  color.red(
                    "replaceServerResData is deprecated, and will be unavilable soon. Use replaceServerResDataAsync instead."
                  ),
                  logUtil.T_ERR
                );
                serverResData =
                  userRule.replaceServerResData(req, res, serverResData) ||
                  serverResData;
                callback();
            ...
            function(callback) {
              userRule.fetchTrafficData &&
                userRule.fetchTrafficData(resourceInfoId, resourceInfo);
              callback();
            }
          ],
          function(err, result) {
...
function connectReqHandler(req, socket, head) {
  ...
  var proxyPort, proxyHost, internalHttpsPort, httpsServerMgrInstance;
      ...
      function(callback) {
        try {
          ...
          conn.on("error", function(e) {
            logUtil.printLog(
              "err when connect to + " + host + " , " + e,
              logUtil.T_ERR
            );
          });
        } catch (e) {
          logUtil.printLog(
            "err when connect to remote https server (__host)".replace(
              /__host/,
              host
            ),
            logUtil.T_ERR
...
function setRules(newRule) {
  if (!newRule) {
    ...
    async.series(functions, function(errors, result) {
      if (!errors) {
        logUtil.printLog(
          color.green("Anyproxy rules initialize finished, have fun!")
        );
      }
    });
